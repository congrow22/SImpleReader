...
export async function updateBookmark(index, memo) {
    if (!currentFilePath) return;
...
function createBookmarkItem(bookmark, index, showFile, fileName) {
    ...
    // Edit & Delete buttons (only for current file bookmarks)
    if (!showFile && index >= 0) {
        const actions = document.createElement('div');
        actions.className = 'bookmark-item-actions';

        const editBtn = document.createElement('button');
        editBtn.className = 'bookmark-item-edit';
        editBtn.title = '수정';
        ...
        editBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            showEditBookmarkDialog(index, bookmark.memo || '');
        });
...
const editDialog = document.getElementById('bookmark-edit-dialog');
const editMemoInput = document.getElementById('bookmark-edit-memo-input');
const editInfoEl = document.getElementById('bookmark-edit-info');
const btnEditConfirm = document.getElementById('btn-bookmark-edit-confirm');
const btnEditCancel = document.getElementById('btn-bookmark-edit-cancel');
const btnEditClose = document.getElementById('btn-bookmark-edit-close');

function showEditBookmarkDialog(index, currentMemo) {
    const bookmark = bookmarks[index];
    if (!bookmark) return;

    const fileType = currentFileType;
    editInfoEl.textContent = formatLocation(bookmark.line || 0, fileType);
    editMemoInput.value = currentMemo;
    editDialog.classList.remove('hidden');

    // Auto-focus and select all text
    setTimeout(() => {
        editMemoInput.focus();
        editMemoInput.select();
    }, 50);
    ...
    function doConfirm() {
        ...
        updateBookmark(index, memo);
        doClose();
    }

    function onOverlay(e) {
        if (e.target === editDialog) doClose();
    }
    ...
    function cleanup() {
        btnEditConfirm.removeEventListener('click', doConfirm);
        btnEditCancel.removeEventListener('click', doClose);
        btnEditClose.removeEventListener('click', doClose);
        editDialog.removeEventListener('click', onOverlay);
        document.removeEventListener('keydown', onKey);
    }

    btnEditConfirm.addEventListener('click', doConfirm);
    btnEditCancel.addEventListener('click', doClose);
    btnEditClose.addEventListener('click', doClose);
    editDialog.addEventListener('click', onOverlay);
    document.addEventListener('keydown', onKey);
}
